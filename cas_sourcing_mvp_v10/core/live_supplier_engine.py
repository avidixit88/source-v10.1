...
def summarize_supplier_rows(detail_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for supplier, g in df.groupby("supplier", dropna=False):
        ...
        row = {
            ...
            "notes": "v10 grouped row. Public prices are trusted only when quantity and price are paired from one structured row.",
            "data_source": "live_supplier_adapter_summary_v10",
        }
        records.append(row)
...
def discover_live_suppliers(
    ...
    """v9 strict supplier-registry sourcing.
    ...
    for supplier in supplier_order:
        ...
        for result in candidates:
            if total_extracted >= max_pages_to_extract or supplier_pages_done >= pages_per_supplier:
                break
            extracted_products = extract_product_rows_from_url(
                cas_number,
                result.url,
            ...
            for extracted in extracted_products:
                ...
                # v10 keep rule: product layer should contain CAS-confirmed pages or meaningful supplier availability state.
                ...
                extracted_rows.append({
                    ...
                    "data_source": "live_extraction_v10_structured_price_rows",
                })
                page_kept = True
            if page_kept:
    ...
    if not detail_df.empty:
        ...
        detail_df = detail_df.sort_values(["cas_exact_match", "source_tier", "listed_price_usd", "extraction_confidence"], ascending=[False, True, True, False])
